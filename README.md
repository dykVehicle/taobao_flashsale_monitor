...
监控淘宝闪购商家后台（饿了么连锁商家后台）的商品状态，自动检测已下架和已售罄的商品，支持导出Excel报表、企业微信通知，以及个人微信/微信群通知（PC微信自动化）。
...
- 📈 **Excel报表导出** - 一键导出监控结果
- 📢 **企业微信通知** - 发现问题商品自动推送告警
- 💬 **微信通知（个人/群）** - 通过PC微信自动化推送到个人微信或微信群
- ⏰ **定时监控** - 支持自定义间隔持续监控
...
| **WeChat通知** | 通过PC微信自动化推送到个人微信/微信群（需安装并登录PC微信） | |
| **目标列表** | 多行文本，每行一个联系人/群名（需完全匹配微信中的名称） | `工作群` |
| **发送正常状态** | 勾选后“无异常”也会推送消息 | |
...
### Q: WSL下运行 build_from_wsl.sh 报错 "Exec format error"？

**原因：** WSL 无法执行 Windows 程序（cmd.exe），通常是互操作性配置问题。

**解决方法：**
1. **推荐**：直接在 Windows 下双击运行项目根目录的 `build.bat`。
2. 尝试重启 WSL (`wsl --shutdown`) 或检查 `/etc/wsl.conf` 中是否启用了 interop。
